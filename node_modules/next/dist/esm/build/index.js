...
import { ACTION, NEXT_ROUTER_PREFETCH_HEADER, RSC_HEADER, RSC_CONTENT_TYPE_HEADER, RSC_VARY_HEADER, NEXT_DID_POSTPONE_HEADER } from "../client/components/app-router-headers";
...
async function writePrerenderManifest(distDir, manifest) {
    ...
    await writeFileUtf8(path.join(distDir, PRERENDER_MANIFEST).replace(/\.json$/, ".js"), `self.__PRERENDER_MANIFEST=${JSON.stringify(JSON.stringify(manifest))}`);
}
...
export default async function build(dir, reactProductionProfiling = false, debugOutput = false, runLint = true, noMangling = false, appDirOnly = false, turboNextBuild = false, turboNextBuildRoot = null, buildMode) {
    ...
    try {
        ...
        await nextBuildSpan.traceAsyncFn(async ()=>{
            ...
            await writeFileUtf8(path.join(distDir, PRERENDER_MANIFEST).replace(/\.json$/, ".js"), `self.__PRERENDER_MANIFEST=${JSON.stringify(JSON.stringify(partialManifest))}`);
            ...
            if (!isGenerateMode) {
                if (runServerAndEdgeInParallel || collectServerBuildTracesInParallel) {
                    ...
                    const { duration: webpackBuildDuration, ...rest } = turboNextBuild ? await turbopackBuild() : await webpackBuild(useBuildWorker, null);
            ...
            if (config.experimental.staticWorkerRequestDeduping) {
                ...
                if (incrementalCacheHandlerPath) {
                    CacheHandler = interopDefault(await import(formatDynamicImportPath(dir, incrementalCacheHandlerPath)).then((mod)=>mod.default || mod));
                }
