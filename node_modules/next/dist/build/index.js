...
const _isdefaultroute = require("../lib/is-default-route");
    ...
    try {
        ...
        await nextBuildSpan.traceAsyncFn(async ()=>{
            ...
            if (appDir) {
                const appPaths = await nextBuildSpan.traceChild("collect-app-paths").traceAsyncFn(()=>(0, _recursivereaddir.recursiveReadDir)(appDir, {
                        pathnameFilter: (absolutePath)=>validFileMatcher.isAppRouterPage(absolutePath) || // For now we only collect the root /not-found page in the app
                            ...
                            validFileMatcher.isRootNotFound(absolutePath) || // Default slots are also valid pages, and need to be considered during path normalization
            ...
            if (config.webpack && config.experimental.webpackBuildWorker === undefined) {
                _log.warn('Custom webpack configuration is detected. When using a custom webpack configuration, the Webpack build worker is disabled by default. To force enable it, set the "experimental.webpackBuildWorker" option to "true". Read more: https://nextjs.org/docs/messages/webpack-build-worker-opt-out');
            }
            ...
            if (!isGenerateMode) {
                if (runServerAndEdgeInParallel || collectServerBuildTracesInParallel) {
                    ...
                    const { duration: webpackBuildDuration, ...rest } = turboNextBuild ? await turbopackBuild() : await (0, _webpackbuild.webpackBuild)(useBuildWorker, null);
            ...
            if (config.experimental.staticWorkerRequestDeduping) {
                ...
                if (incrementalCacheHandlerPath) {
                    CacheHandler = (0, _interopdefault.interopDefault)(await import((0, _formatdynamicimportpath.formatDynamicImportPath)(dir, incrementalCacheHandlerPath)).then((mod)=>mod.default || mod));
                }
                                    ...
                                    if (pageType === "app" && originalAppPath) {
                                        ...
                                        if ((0, _isedgeruntime.isEdgeRuntime)(pageRuntime)) {
                                            ...
                                            if (appConfig.revalidate !== 0) {
                                                var _workerResult_prerenderRoutes, _workerResult_prerenderRoutes1;
                                                ...
                                                const isEmptyGenerateStaticParams = ((_workerResult_prerenderRoutes1 = workerResult.prerenderRoutes) == null ? void 0 : _workerResult_prerenderRoutes1.length) === 0;
                                                if (config.output === "export" && isDynamic) {
                                                    if (isEmptyGenerateStaticParams) {
                                                        throw new Error(`Page "${page}"'s "generateStaticParams()" returned an empty array, which is not allowed with "output: export" config.`);
                ...
                await staticGenerationSpan.traceAsyncFn(async ()=>{
                    ...
                    for (const [originalAppPath, routes] of appStaticPaths){
                        ...
                        routes.forEach((route)=>{
                            ...
                            if ((0, _isdefaultroute.isDefaultRoute)(page)) return;
